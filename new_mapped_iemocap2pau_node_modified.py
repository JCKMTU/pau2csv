...
def talker():
    ...
    head_data = np.loadtxt(sys.argv[1], delimiter=',')

    face_data = np.loadtxt(sys.argv[2], delimiter=',')  
    
    phon_data = np.loadtxt(sys.argv[3], delimiter=',')


    face_data = np.delete(face_data, np.s_[13:27], axis=0)

    rospy.loginfo("loaded file with %d frames." % len(data))

    angle_range = 45.0

    for head_frame, face_frame, phon_data in zip(head_data, face_data, phon_data):
        msg = pau()

        msg.m_headRotation.x = head_frame[0]
        msg.m_headRotation.y = head_frame[1]
        msg.m_headRotation.z = head_frame[2]
        msg.m_headRotation.w = head_frame[3]
        ...
        msg.m_coeffs = face_frame[16:]
        
        print("conf: %f speak: %f" % (face_frame[-2], face_frame[-1]))
